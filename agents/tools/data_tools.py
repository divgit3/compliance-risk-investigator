...
def get_top_anomalous_features(hcp_id: str) -> dict:
    ...
    Returns the top_n features by global Pearson |r| importance (from
    feature_importance.csv), excluding features that were not used in
    Isolation Forest training. For each feature, shows the HCP's actual
    value, the population mean, and whether the HCP is high or low.

    Note: Uses global Pearson proxy importance. Per-HCP SHAP values will
    replace this in Task 3.7.
    """
    hcp_id = hcp_id.strip().strip("'\"")
    if "," in hcp_id:
        hcp_id = hcp_id.split(",")[0].strip().strip("'\"")
    try:
        ...
        # Filter out IF-excluded features, then take top_n
        top_n = 5
        ...
        for _, fi_row in fi_filtered.iterrows():
            ...
            # Flag values that appear z-score scaled (raw features are
            # positive counts or 0-1 rates — negatives below -1.0 indicate
            # a scaled column leaked from feature_store_raw)
            value_note = (
                "value may be z-score scaled — check feature_store_raw source"
                if hcp_val < -1.0 else None
            )   
            entry = {
                "feature_name":     feat,
        ...
        return {
            "hcp_id":   hcp_id,
            "top_n":    len(results),
            "note":     "Pearson |r| proxy importance — SHAP per-HCP values coming in Task 3.7",
            "features": results,
        }
